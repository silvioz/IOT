function [BER]= functionBER2(Nbits, SNRdB)
...
Symbols2(Symbols == (1+ 1i) / sqrt(2)) = -(1+ 1i) / sqrt(2);
Symbols2(Symbols == (1- 1i) / sqrt(2)) = -(1- 1i) / sqrt(2);
Symbols2(Symbols == (-1- 1i) / sqrt(2)) = (1- 1i) / sqrt(2);
Symbols2(Symbols == (-1+ 1i) / sqrt(2)) = (1+ 1i) / sqrt(2);
...
for i = 1:dimOutput
    ...
    w = (sigma/(sqrt(2)))*w1 + 1i*(sigma/(sqrt(2)))*w2;
    Noisy = Symbols2 + w;
    ...
    receivedBits1(angles < (pi/2) & angles > 0) = 0;
    receivedBits2(angles < (pi/2) & angles > 0) = 1;
    receivedBits1(angles > (pi/2) & angles < pi) = 1;
